...
// ranNumGen = Math.floor(Math.random() * 20);
// const myArray = ['bob', 'alice', 'john'];

function selectRandom(anyArray) {
    ranNumGen = Math.floor(Math.random() * anyArray.length);
    return anyArray[ranNumGen];
}
...
function revealMystery(anyObject) {
    return `${anyObject.suspect.firstName} ${anyObject.suspect.lastName} killed Mr. Boddy using the ${anyObject.weapon.name} in the ${anyObject.room}!`;
} 
